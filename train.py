...
# We need a path to mushroom training images and test images
TRAINING_FOLDER = os.getcwd() + '/training_images'
TEST_FOLDER = os.getcwd() + "/test_images/"
# where to save the trained graph
OUTPUT_GRAPH = os.getcwd() + '/' + 'trained_graph.pb'
...
OUTPUT_LABELS = os.getcwd() + '/' + 'trained_labels.txt'
# where to save summary logs for TensorBoard
TENSORBOARD_FOLDER = os.getcwd() + '/' + 'tensorboard_logs'
# this is the number of steps that will run for training.
...
def main():
    ...
    image_lists = create_image_lists(TRAINING_FOLDER, TESTING_PERCENTAGE, VALIDATION_PERCENTAGE)
    class_count = len(image_lists.keys())
    if class_count == 0:
        tf.logging.error('No valid folders of images found at ' + TRAINING_FOLDER)
        return -1
    if class_count == 1:
        tf.logging.error('Only one valid folder of images found at ' + TRAINING_FOLDER + ' - multiple classes are needed for classification.')
        return -1
    ...
    with tf.Session(graph=graph) as sess:
        ...
        if doDistortImages:
            ...
            cache_bottlenecks(sess, image_lists, TRAINING_FOLDER, BOTTLENECK_DIR, jpeg_data_tensor, decoded_image_tensor,
        ...
        validation_writer = tf.summary.FileWriter(TENSORBOARD_FOLDER + '/validation')
        ...
        for i in range(TRAINING_STEPS):
            if doDistortImages:
                (train_bottlenecks, train_ground_truth) = get_random_distorted_bottlenecks(sess, image_lists, TRAIN_BATCH_SIZE, 'training',
                                                                                           TRAINING_FOLDER, distorted_jpeg_data_tensor,
                ...
                (train_bottlenecks, train_ground_truth, _) = get_random_cached_bottlenecks(sess, image_lists, TRAIN_BATCH_SIZE, 'training',
                                                                                           BOTTLENECK_DIR, TRAINING_FOLDER, jpeg_data_tensor,
        ...
        test_bottlenecks, test_ground_truth, test_filenames = (get_random_cached_bottlenecks(sess, image_lists, TEST_BATCH_SIZE, 'testing', BOTTLENECK_DIR,
                                                                                             TRAINING_FOLDER, jpeg_data_tensor, decoded_image_tensor, resized_image_tensor,
...
def prepare_file_system():
    if tf.gfile.Exists(TENSORBOARD_FOLDER):
        tf.gfile.DeleteRecursively(TENSORBOARD_FOLDER)
    tf.gfile.MakeDirs(TENSORBOARD_FOLDER)
...
def create_model_info(architecture):
    ...
    if architecture == 'inception_v3':
        ...
        if is_quantized:
            ...
            model_folder_name = ('mobilenet_v1_' + version_string + '_' + size_string + '_quantized_frozen')
            ...
            model_folder_name = 'mobilenet_v1_' + version_string + '_' + size_string
            model_base_name = 'frozen_graph.pb'
        bottleneck_tensor_size = 1001
        input_width = int(size_string)
        input_height = int(size_string)
        input_depth = 3
        model_file_name = os.path.join(model_folder_name, model_base_name)
        input_mean = 127.5
        input_std = 127.5
    else:
        tf.logging.error("Couldn't understand architecture name '%s'", architecture)
...
def create_model_graph(model_info):
    with tf.Graph().as_default() as graph:
        model_path = os.path.join(MODEL_FOLDER, model_info['model_file_name'])
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    #This is where we will make our list of images for training
    #Creates a dictionary of for our labels
